...
public final class MainLevelAvailabilityChunkSource implements SkyesightClientChunkSource {
    private final LongSet trackedChunks = new LongOpenHashSet();

    @Override
    public void updateReadyChunks(
            ClientLevel skyesightLevel,
            ChunkTracker tracker,
            Vec3 cameraPosition,
            int radius
    ) {
        ClientLevel sourceLevel = Minecraft.getInstance().level;
        ...
        for (int dz = -radius - 1; dz <= radius + 1; dz++) {
            for (int dx = -radius - 1; dx <= radius + 1; dx++) {
                ...
                if (sourceLevel.getChunkSource().getChunk(chunkX, chunkZ, false) != null) {
        ...
        for (long packed : wanted) {
            if (this.trackedChunks.add(packed)) {
                tracker.onChunkStatusAdded(ChunkPos.getX(packed), ChunkPos.getZ(packed), 3);
            }
        }
        ...
        for (long packed : removed) {
            ...
            tracker.onChunkStatusRemoved(ChunkPos.getX(packed), ChunkPos.getZ(packed), 3);
        }
    }
}